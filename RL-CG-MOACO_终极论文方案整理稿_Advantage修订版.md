...
BlockPenalty(v)=
\frac{
\sum_{u\in N(v)\cap A_t}p(u)
}
{
\sum_{u\in A_t}p(u)+\epsilon
}
\]

含义是：

> 当前节点 \(v\) 虽然可以选，但如果选了它，会屏蔽掉多少后续高收益候选节点。

由于：

\[
N(v)\cap A_t\subseteq A_t
\]
...
42.         Apply lightweight insertion-replacement local search
